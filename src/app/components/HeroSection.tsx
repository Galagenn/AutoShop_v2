    ...
    <section className="relative pb-6 md:pb-10 pt-6 md:pt-8">
      ...
      <div className="container-page">
        <div className="hero-filters">
          {/* Brand dropdown */}
          <div ref={brandRef} className={`hero-dd ${openBrand ? 'open' : ''}`} style={{ width: 160 }} onClick={() => setOpenBrand(!openBrand)}>
          ...
          <div className="hero-filter" style={{ width: 150 }}>
            <input className="hero-input" placeholder="Модель" value={model} onChange={(e) => setModel(e.target.value)} />
          </div>
          {/* Year dropdown */}
          <div ref={yearRef} className={`hero-dd ${openYear ? 'open' : ''}`} style={{ width: 140 }} onClick={() => setOpenYear(!openYear)}>
          ...
          <div className="hero-filter" style={{ width: 140 }}>
      ...
      <div className="container-page mt-4 md:mt-8">
        <div className="relative hero-bottom">
          {/* Accent line */}
          <div className="absolute left-0 top-4 bottom-4 w-[2px] bg-[#C8BF2F] rounded-r" />
          ...
          <div className="pl-4 md:pl-6 grid gap-4 md:gap-6">
            ...
            <div className="hero-header-row">
              <div className="flex-1">
                <h3 className="text-xl md:text-2xl font-bold tracking-tight mb-2 md:mb-3">{currentCar.title}</h3>
            ...
            {/* Metrics row */}
            <div className="grid grid-cols-3 gap-4 md:gap-6 pt-4 border-t border-[#2a2a2a]">
  ...
  return (
    <div className="flex flex-col items-start gap-2">
      <div className="flex items-center gap-2 mb-1">
        <GearIcon className="w-4 h-4 md:w-5 md:h-5 text-white/50" />
        <div className="h-[1px] w-8 md:w-12 bg-[#C8BF2F]/40" />
      </div>
      <div className="flex flex-col gap-1">
        <span className="text-2xl md:text-3xl leading-none font-bold tracking-tight">{value}</span>
        <span className="text-[10px] md:text-xs leading-none text-white/60 uppercase tracking-wider">{label}</span>
      </div>
    </div>
  );
}
